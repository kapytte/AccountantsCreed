...
//CONTROLS TRADE FUNCTIONS, AND MANAGES PRICE OF GOODS
public class ShopSystem : MonoBehaviour 
{
	...
	void Start()
	{
		...
		if (scene == 1)
		{
			StartCoroutine(Outpost1());
		}
	}

	IEnumerator Outpost1()
	{
	...
	public void CargoTextAdd()
	{
		...
		while (i < inv.Count)
		{
			...
			{
				string n;
				n = cargo[i].name[0].ToString();
				inv[i].text = n;

				if (n == "W")
				{
					inv[i].color = caravan.GetComponent<Caravan>().yellow;
				}
				else if (n == "F")
				{
					inv[i].color = caravan.GetComponent<Caravan>().lightBlue;
				}
				else if (n == "L")
				{
					inv[i].color = caravan.GetComponent<Caravan>().darkGreen;
				}
				else if (n == "I")
				{
					inv[i].color = caravan.GetComponent<Caravan>().ironGrey;
				}
...
//		print (cargo.Count);
		...
		if (town.GetComponentInChildren<Town>().lumber > 0 && cargo.Count < 10 && questGiver.GetComponent<MultipleChoice>().goldN >= Mathf.CeilToInt (lumberPrice))
			bL.interactable = true;
		else
			bL.interactable = false;

		if (town.GetComponentInChildren<Town>().fish > 0 && cargo.Count < 10 && questGiver.GetComponent<MultipleChoice>().goldN >= Mathf.CeilToInt (fishPrice))
			bF.interactable = true;
		else
			bF.interactable = false;

		if (town.GetComponentInChildren<Town>().iron > 0 && cargo.Count < 10 && questGiver.GetComponent<MultipleChoice>().goldN >= Mathf.CeilToInt (ironPrice))
			bI.interactable = true;
		else
			bI.interactable = false;

		if (town.GetComponentInChildren<Town>().wheat > 0 && cargo.Count < 10 && questGiver.GetComponent<MultipleChoice>().goldN >= Mathf.CeilToInt (wheatPrice))
	...
	public void Outpost()
	{
		for (int i = 0; i < outpostList.Count; i++)
		{
			...
			outpostList[i].wheatPrice = ((200 - towns[i].GetComponentInChildren<Town>().wheat) * 0.025f) + 2;
			outpostList[i].wheatPriceT.text = Mathf.CeilToInt(outpostList[i].wheatPrice).ToString() + "G";

			outpostList[i].fishPrice = ((150 - towns[i].GetComponentInChildren<Town>().fish) * 0.066f) + 6;
			outpostList[i].fishPriceT.text = Mathf.CeilToInt(outpostList[i].fishPrice).ToString() + "G";

			outpostList[i].lumberPrice = ((100 - towns[i].GetComponentInChildren<Town>().lumber) * 0.15f) + 10;
			outpostList[i].lumberPriceT.text = Mathf.CeilToInt(outpostList[i].lumberPrice).ToString() + "G";

			outpostList[i].ironPrice = ((50 - towns[i].GetComponentInChildren<Town>().iron) * 0.5f) + 15;
			outpostList[i].ironPriceT.text = Mathf.CeilToInt(outpostList[i].ironPrice).ToString() + "G";
		}
	}
	...
	void Price()
	{
		fishPrice = ((150 - town.GetComponentInChildren<Town>().fish) * 0.066f) + 6;
		lumberPrice = ((100 - town.GetComponentInChildren<Town>().lumber) * 0.15f) + 10;
		ironPrice = ((50 - town.GetComponentInChildren<Town>().iron) * 0.5f) + 15;
		wheatPrice = ((200 - town.GetComponentInChildren<Town>().wheat) * 0.025f) + 2;

		uFish.text = town.GetComponentInChildren<Town>().fish.ToString();
		uLumber.text = town.GetComponentInChildren<Town>().lumber.ToString();
		UIron.text = town.GetComponentInChildren<Town>().iron.ToString();
		uGold.text = town.GetComponentInChildren<Town>().wheat.ToString();
	...
	{
		town.GetComponentInChildren<Town>().lumber -= 1;
	...
	public void BuyFish()
	{
		town.GetComponentInChildren<Town>().fish -= 1;
	...
	public void BuyIron()
	{
		town.GetComponentInChildren<Town>().iron -= 1;
	...
	public void BuyGold()
	{
		if (scene == 1)
		{
			town.GetComponentInChildren<Town>().wheat -= 1;
			MercsLeave();
		}
